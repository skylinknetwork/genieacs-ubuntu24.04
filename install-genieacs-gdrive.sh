...
# MongoDB 7.0 + mongosh dari bundle Google Drive
set -euo pipefail
...
# =====================================================
# 3. Install MongoDB 7.0 dari bundle Google Drive
# =====================================================
echo "=== [3/6] Install MongoDB 7.0 dari Google Drive ==="

MONGO_URL="https://drive.google.com/uc?export=download&id=1KszA1AMopHJ1xb7d10mrBmM4P-m28TfA"
WORKDIR="/tmp/mongodb-bundle"

sudo rm -rf "${WORKDIR}"
mkdir -p "${WORKDIR}"
cd "${WORKDIR}"

echo "[INFO] Download bundle MongoDB (bisa agak lama)..."
# -L supaya mengikuti redirect Google Drive
curl -fsSL -L "${MONGO_URL}" -o mongodb-org-7.0-bundle.tar.gz

echo "[INFO] Extract paket .deb..."
tar xzf mongodb-org-7.0-bundle.tar.gz

echo "[INFO] Install semua paket MongoDB (.deb)..."
sudo dpkg -i ./*.deb || sudo apt -f install -y

echo "[INFO] Enable & start mongod..."
